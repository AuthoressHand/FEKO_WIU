 ...
 *
 * @author rms130
 * @dateUpdated - 4/1/2018 - added the AI to the MapGUI, added calculating for shortest distance to allychar - Rose
 */
public class LevelGUITestingDoubleMatrix extends javax.swing.JFrame {
    ...
    public LevelGUITestingDoubleMatrix() {
        initComponents();
    }
    
    public LevelGUITestingDoubleMatrix(int level, Party party, CharacterModGUI cmGUI) {
    ...
    private void enemyAI() {
        //find where the closest AllyChar is
        
        //loop through each of the different buttons to get them all to move/ attack
        ...
        //turn it back over to the player
    }
    
    //Recalculates the GridTiles based on where the 
    private void recalculateLocationOfAllyChars(int x, int y) {
        //resets all of the distances starting at the newest location and working around the 
        
        //after calculating for lowerGrid, replace for upperGrid    
        /*for (int i = 0; i < lowerGrid.length; i++) {
            for (int j = 0; j < lowerGrid[i].length; j++)
                upperGrid[i][j].setDistance(lowerGrid[i][j].getDistance());
        }*/
    }
    ...
    private void performAction(Component mapBoundary, Component character, JLayeredPane characterHP, Party party, int armyPos) {
        ...
        if(isMouseWithinComponent(mapBoundary)) {
            ...
            for(int j = 0; j < lowerGrid.length - 1; j++) 
            for (int i = 0; i < lowerGrid[j].length - 1; i++) {
                cleanUpEnemyTrace(j, i);
                //If the mouse is within the GridTile and it is accessible, move Character to the GridTile and clean up its trace
                if(isMouseWithinComponent(lowerGrid[j][i].getTile()) && lowerGrid[j][i].isAccessible() && lowerGrid[j][i].getTile().getIcon().toString().equals(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/bluePositionMarker.png")).toString())) {
                    //Sets Character location and adds Character to the GridTile
                    checkForEndTurn(party);
                    character.setLocation(lowerGrid[j][i].getTile().getX(), lowerGrid[j][i].getTile().getY() + TopBorderStats.getHeight());
                    characterHP.setLocation(lowerGrid[j][i].getTile().getX(), lowerGrid[j][i].getTile().getY() + TopBorderStats.getHeight() + 70);
                    
                    if(character.getLocation() == charInitialPoint) {
                        break;
                    }
                    lowerGrid[j][i].addCharacter(party.getArmyChar(armyPos), party);
                    
                    //Cleans up Character trace
                    for (GridTile[] grid: lowerGrid) {
                ...
                else if((isMouseWithinComponent(lowerGrid[j][i].getTile())) && lowerGrid[j][i].isAccessible() == false && lowerGrid[j][i].isOccupied() && !lowerGrid[j][i].getTile().getIcon().toString().equals(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/empty.png")).toString()) && !lowerGrid[j][i].getTile().getIcon().toString().equals(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/redPositionMarker.png")).toString())) {
                    checkCharacterDamage(party, lowerGrid[j][i], character, characterHP, armyPos);
                    resetPositionMarkers();
                    checkCharacterDeath(j, i, party, party.getArmyChar(armyPos), armyPos);
                    checkStageOver();   
                    checkForEndTurn(party);
                } 
                //Last Condition: If the GridTile is not accessible, reset the position of the Character.
                else if(lowerGrid[j][i].isAccessible() == false){
    ...
    private void cleanUpAllyTrace(Component character, Party party, int armyPos) {
        if(party == allyParty) {
            for(int i = 0; i < upperGrid.length - 1; i++) {
                for (int j = 0; j < upperGrid[i].length - 1; j++) {
                    if(upperGrid[i][j].getTile().getIcon().toString().equals(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/allyPositionMarker.png")).toString()) && upperGrid[i][j].getCharacter() != null && upperGrid[i][j].getCharacter().equals(party.getArmyChar(armyPos))) {
                        upperGrid[i][j].getTile().setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/empty.png")));
                        upperGrid[i][j].removeCharacter();
                    }
                }
            }
            for(int i = 0; i < upperGrid.length - 1; i++) {
                for (int j = 0; j < upperGrid[i].length - 1; j++) {
                    if(upperGrid[i][j].getTile().getLocation().equals(new Point(character.getX(), character.getY() - TopBorderStats.getHeight()))) {
                        upperGrid[i][j].getTile().setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/allyPositionMarker.png")));
                        return;
                    }
                }
            }
    ...
    private void cleanUpEnemyTrace(int i, int j) {
        if(upperGrid[i][j].getTile().getIcon().toString().equals(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/enemyPositionMarker.png")).toString())) {
            upperGrid[i][j].getTile().setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/empty.png")));
        }
    }
        ...
        if(isMouseWithinComponent(MainContainer)) {
            ...
            if(party == allyParty) {    
                for(int j = 0; j < upperGrid.length - 1; j++) {
                    for (int k = 0; k < upperGrid[j].length - 1; k++) {
                        cleanUpEnemyTrace(j, k);
                        //Moves ally marker
                        if(upperGrid[j][k].getTile().getBounds().contains(mouseLocation.x + character.getWidth()/2, mouseLocation.y - TopBorderStats.getHeight() + character.getHeight()/2) && upperGrid[j][k].isAccessible() && lowerGrid[j][k].getTile().getIcon().toString().equals(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/bluePositionMarker.png")).toString())) {
                            upperGrid[j][k].getTile().setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/allyPositionMarker.png")));
                            upperGrid[j][k].addCharacter(party.getArmyChar(armyPos), party);
                            tileSound();

                            //Cleans up character trace
                            for(int i = 0; i < upperGrid.length - 1; i++) {
                                for (int p = 0; p < upperGrid[i].length - 1; p++) {
                                    if((upperGrid[i][p].getTile().getIcon().toString().equals(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/enemyPositionMarker.png")).toString())||upperGrid[i][p].getTile().getIcon().toString().equals(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/allyPositionMarker.png")).toString())) && !lowerGrid[i][p].getTile().equals(lowerGrid[j][k].getTile()) && upperGrid[i][p].getCharacter() != null && upperGrid[i][k].getCharacter().equals(party.getArmyChar(armyPos))) {
                                        upperGrid[i][p].getTile().setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/empty.png")));
                                        upperGrid[i][p].removeCharacter();
                                    }
                                }
                            }
                        }
                        //Adds battle icon above enemies when hovered over
                        else if(upperGrid[j][k].getTile().getBounds().contains(mouseLocation.x + character.getWidth()/2, mouseLocation.y - TopBorderStats.getHeight() + character.getHeight()/2) && lowerGrid[j][k].getTile().getIcon().toString().equals(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/darkRedPositionMarker.png")).toString())) {
                            for (GridTile[] gta: upperGrid) {
                                for(GridTile gt: gta) {
                            ...
                            updateBattleStatsLayer(party.getArmyChar(armyPos),lowerGrid[j][k].getCharacter());
                            MapLayer.moveToFront(AttackPopUp);
                            AttackPopUp.setVisible(true);
                            AttackPopUp.setLocation(new Point(upperGrid[j][k].getTile().getX() + character.getWidth()/4, upperGrid[j][k].getTile().getY() + TopBorderStats.getHeight() - character.getHeight()/2));
                            upperGrid[j][k].getTile().setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/enemyPositionMarker.png")));

                            //Cleans up character trace
                            for(int i = 0; i < upperGrid.length - 1; i++) {
                                for (int p = 0; p < upperGrid[i].length - 1; p++)
                                    if(!upperGrid[i][p].getTile().getLocation().equals(new Point(charInitialPoint.x,charInitialPoint.y - TopBorderStats.getHeight())) && upperGrid[i][p].getTile().getIcon().toString().equals(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/allyPositionMarker.png")).toString()) && !lowerGrid[i][p].getTile().equals(lowerGrid[j][k].getTile()) && upperGrid[i][p].getCharacter() != null && upperGrid[i][p].getCharacter().equals(party.getArmyChar(armyPos))) {
                                        upperGrid[i][p].getTile().setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/empty.png")));
                                        upperGrid[i][p].removeCharacter();
                                    }
                            }
                        ...
                        else if(upperGrid[j][k].getTile().getBounds().contains(mouseLocation.x + character.getWidth()/2, mouseLocation.y - TopBorderStats.getHeight() + character.getHeight()/2) && (lowerGrid[j][k].getTile().getIcon().toString().equals(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/empty.png")).toString()) || lowerGrid[j][k].getTile().getIcon().toString().equals(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/redPositionMarker.png")).toString()))) {
                            for (GridTile[] gta: upperGrid)
                                for(GridTile gt: gta) {
                                    if(gt.getTile().getLocation().equals(new Point(charInitialPoint.x,charInitialPoint.y - TopBorderStats.getHeight()))) {
                                        gt.getTile().setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/allyPositionMarker.png")));
    ...
    private void checkCharacterDeath(int gridPosX, int gridPosY, Party party, Char damageDealer, int armyPos) {
        if(lowerGrid[gridPosX][gridPosY].getCharacter().getCurrentHP() < 1) {
            ...
            for(int i = 0; i < characters.length; i++) {
                if(characters[i].getIcon() != null && lowerGrid[gridPosX][gridPosY].getCharacter() != null && characters[i].getIcon().toString().equals(lowerGrid[gridPosX][gridPosY].getCharacter().getImg().toString())) {
                    deathSound();
                    characters[i].setVisible(false);
                    charactersHP[i].setVisible(false);
                    lowerGrid[gridPosX][gridPosY].getTile().setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/empty.png")));
                    upperGrid[gridPosX][gridPosY].getTile().setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/empty.png")));
                    lowerGrid[gridPosX][gridPosY].removeCharacter();
                }
            }
        }
    }
    ...
    private void showPossibleMoves(Party party, int partyPosition, boolean reset) {
        ...
        int initialGridTileX = 0;
        int initialGridTileY = 0;
        for(int i = 0; i < lowerGrid.length ; i++) {
            for (int j = 0; j < lowerGrid[i].length; j++)
                if(lowerGrid[i][j].getCharacter() != null && lowerGrid[i][j].getCharacter().equals(party.getArmyChar(partyPosition))) {
                    initialGridTileX = i;
                    initialGridTileY = j;
                }
        }
        ...
        lowerGrid[initialGridTileX][initialGridTileY].getTile().setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/bluePositionMarker.png")));
        ...
        */
        ...
        if(walkRange >= 1) {
            //right
            if (initialGridTileY != 5 && initialGridTileY + 1 > -1 && initialGridTileY + 1 < 6) {
                setPositionMarker(party, initialGridTileX, initialGridTileY + 1, 0);
            }
            //left
            if (initialGridTileY != 0 && initialGridTileY - 1 > -1 && initialGridTileY - 1 < 6) {
                setPositionMarker(party, initialGridTileX, initialGridTileY - 1, 0);
            }
            //up
            if (initialGridTileX - 1 > -1 && initialGridTileX - 1 < lowerGrid.length) {
                setPositionMarker(party, initialGridTileX - 1, initialGridTileY, 0);
            }
            //down
            if (initialGridTileX + 1 > -1 && initialGridTileX + 1 < lowerGrid.length) {
                setPositionMarker(party, initialGridTileX + 1, initialGridTileY, 0);
        ...
        if(walkRange >= 2) {
            //right right
            if (initialGridTileY != 5 && initialGridTileY + 2 > -1 && initialGridTileY + 2 < 6) {
                setPositionMarker(party, initialGridTileX, initialGridTileY + 2, 0);
            }
            //left left
            if (initialGridTileY != 0 && initialGridTileY - 2 > -1 && initialGridTileY - 2 < 6) {
                setPositionMarker(party, initialGridTileX, initialGridTileY - 2, 0);
            }
            //right down
            if (initialGridTileY != 5 && initialGridTileY + 1 > -1 && initialGridTileY + 1 < 6 && 
                    initialGridTileX + 1 > -1 && initialGridTileX + 1 < lowerGrid.length) {
                setPositionMarker(party, initialGridTileX + 1, initialGridTileY + 1, 0);
            }
            //right up
            if (initialGridTileY != 5 && initialGridTileY + 1 > -1 && initialGridTileY + 1 < 6 && 
                    initialGridTileX - 1 > -1 && initialGridTileX - 1 < lowerGrid.length) {
            ...
            //up up
            if (initialGridTileX - 2 > -1 && initialGridTileX - 2 < lowerGrid.length) {
                setPositionMarker(party, initialGridTileX - 2, initialGridTileY, 0);
            }
            //down down
            if (initialGridTileX + 2 > -1 && initialGridTileX + 2 < lowerGrid.length) {
                setPositionMarker(party, initialGridTileX + 2, initialGridTileY, 0);
    ...
    private void setPositionMarker(Party party, int posX, int posY, int posRange) {
        Party otherParty;
        if(party == allyParty) {
        ...
        if(lowerGrid[posX][posY].isAccessible() && lowerGrid[posX][posY].getCharacter() == null) {
            if(!lowerGrid[posX+posRange][posY+posRange].getTile().getIcon().toString().equals(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/redPositionMarker.png")).toString()) && !lowerGrid[posX+posRange][posY+posRange].getTile().getIcon().toString().equals(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/darkRedPositionMarker.png")).toString()))
                lowerGrid[posX][posY].getTile().setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/bluePositionMarker.png")));
            else
                lowerGrid[posX][posY].getTile().setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/redPositionMarker.png")));
        } else if(!lowerGrid[posX][posY].isAccessible() && lowerGrid[posX][posY].getParty() != null && lowerGrid[posX][posY].getParty().equals(otherParty)) {
            if(!lowerGrid[posX+posRange][posY+posRange].getTile().getIcon().toString().equals(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/redPositionMarker.png")).toString()) && !lowerGrid[posX+posRange][posY+posRange].getTile().getIcon().toString().equals(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/darkRedPositionMarker.png")).toString()))
                lowerGrid[posX][posY].getTile().setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/darkRedPositionMarker.png")));
            else
                lowerGrid[posX][posY].getTile().setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/redPositionMarker.png")));
            
        } else if(!lowerGrid[posX][posY].isAccessible() && lowerGrid[posX][posY].getParty() != null && lowerGrid[posX][posY].getParty().equals(party)) {
            lowerGrid[posX][posY].getTile().setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/darkBluePositionMarker.png")));
        } else if(!lowerGrid[posX][posY].isAccessible() && lowerGrid[posX][posY].getCharacter() == null){
            lowerGrid[posX][posY].getTile().setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/redPositionMarker.png")));
        }
    }
    
    //Initializes LevelGUITestingDoubleMatrix variables before user input
    private void initMapGUI() {
        ...
        for(int i = 0; i < 8; i++) {
            ...
            for(int j = 0; j < lowerGrid.length; j++) {
                for (int k = 0; k < lowerGrid[j].length; k++) {
                    if(isCharacterWithinTile(characters[i], lowerGrid[j][k].getTile())) {
                        if(i < 4) {
                            upperGrid[j][k].getTile().setIcon(new javax.swing.ImageIcon(getClass().getResource("/img/UIMenu/allyPositionMarker.png")));
                            lowerGrid[j][k].addCharacter(allyParty.getArmyChar(i), allyParty);
                        }else {
                            lowerGrid[j][k].addCharacter(enemyParty.getArmyChar(i-4), enemyParty);
                        }
                    }
                }
            }
    ...
    private void setLevelValues() {
        if(level == 1) {
            ...
            LevelGUITestingDoubleMatrix.lowerGrid = new GridTile[][]{{new GridTile(pos00,true), new GridTile(pos01,false), new GridTile(pos02,false), new GridTile(pos03,true), new GridTile(pos04,true), new GridTile(pos05,false)}, 
            ...
            LevelGUITestingDoubleMatrix.upperGrid = new GridTile[][]{{new GridTile(pos1,true), new GridTile(pos2,false), new GridTile(pos3,false), new GridTile(pos4,true), new GridTile(pos5,true), new GridTile(pos6,false)}, 
        ...
        if(level == 2) {
            ...
            LevelGUITestingDoubleMatrix.lowerGrid = new GridTile[][]{{new GridTile(pos00,false), new GridTile(pos01,true), new GridTile(pos02,true), new GridTile(pos03,true), new GridTile(pos04,true), new GridTile(pos05,false)}, 
            ...
            LevelGUITestingDoubleMatrix.upperGrid = new GridTile[][]{{new GridTile(pos1,false), new GridTile(pos2,true), new GridTile(pos3,true), new GridTile(pos4,true), new GridTile(pos5,true), new GridTile(pos6,false)}, 
        ...
        if(level == 3) {
            ...
            LevelGUITestingDoubleMatrix.lowerGrid = new GridTile[][]{{new GridTile(pos00,true), new GridTile(pos01,true), new GridTile(pos02,true), new GridTile(pos03,true), new GridTile(pos04,true), new GridTile(pos05,true)}, 
                                        {new GridTile(pos10,true), new GridTile(pos11,true), new GridTile(pos12,true), new GridTile(pos13,true), new GridTile(pos14,true), new GridTile(pos15,true)}, 
                                        {new GridTile(pos20,true), new GridTile(pos21,true), new GridTile(pos22,true), new GridTile(pos23,true), new GridTile(pos24,true), new GridTile(pos25,true)}, 
                                        {new GridTile(pos30,true), new GridTile(pos31,true), new GridTile(pos32,true), new GridTile(pos33,true), new GridTile(pos34,true), new GridTile(pos35,true)}, 
                                        {new GridTile(pos40,true), new GridTile(pos41,true), new GridTile(pos42,true), new GridTile(pos43,true), new GridTile(pos44,true), new GridTile(pos45,true)}, 
            ...
            LevelGUITestingDoubleMatrix.upperGrid = new GridTile[][]{{new GridTile(pos1,true), new GridTile(pos2,true), new GridTile(pos3,true), new GridTile(pos4,true), new GridTile(pos5,true), new GridTile(pos6,true)}, 
                                        {new GridTile(pos16,true), new GridTile(pos17,true), new GridTile(pos18,true), new GridTile(pos19,true), new GridTile(pos26,true), new GridTile(pos27,true)}, 
                                        {new GridTile(pos28,true), new GridTile(pos29,true), new GridTile(pos36,true), new GridTile(pos37,true), new GridTile(pos38,true), new GridTile(pos39,true)}, 
                                        {new GridTile(pos46,true), new GridTile(pos47,true), new GridTile(pos48,true), new GridTile(pos49,true), new GridTile(pos56,true), new GridTile(pos57,true)}, 
                                        {new GridTile(pos58,true), new GridTile(pos59,true), new GridTile(pos66,true), new GridTile(pos67,true), new GridTile(pos68,true), new GridTile(pos69,true)}, 
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(LevelGUITestingDoubleMatrix.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new LevelGUITestingDoubleMatrix().setVisible(true);
            }
        });
    }
